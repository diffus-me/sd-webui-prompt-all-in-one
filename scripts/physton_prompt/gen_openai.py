...
def gen_openai(p: Paths, messages, api_config):
    import openai
    from distutils.version import LooseVersion
    api_config = unprotected_translate_api_config(p, 'chatgpt_key', api_config)
    openai.api_base = api_config.get('api_base', 'https://api.openai.com/v1')
    openai.api_key = api_config.get('api_key', '')
    model = api_config.get('model', 'gpt-3.5-turbo')
    if not openai.api_key:
        raise Exception(get_lang(p, 'is_required', {'0': 'API Key'}))
    if not messages or len(messages) == 0:
        raise Exception(get_lang(p, 'is_required', {'0': 'messages'}))
    ...
    if len(completion.choices) == 0:
        raise Exception(get_lang(p, 'no_response_from', {'0': 'OpenAI'}))
